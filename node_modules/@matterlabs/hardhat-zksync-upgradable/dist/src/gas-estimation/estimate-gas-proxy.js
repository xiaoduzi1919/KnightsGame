...
const deploy_1 = require("../proxy-deployment/deploy");
async function deployProxyAdminLocally(adminFactory) {
    const mockContract = await (0, deploy_1.deploy)(adminFactory);
...
function makeEstimateGasProxy(hre) {
    return async function estimateGasProxy(deployer, artifact, args = [], opts = {}, quiet = false) {
        ...
        if (chainId === 270) {
            const adminFactory = await (0, deploy_proxy_admin_1.getAdminFactory)(hre, deployer.zkWallet);
            mockImplAddress = await deployProxyAdminLocally(adminFactory);
            // videti tamo u openzeppeln
            data = (0, utils_general_1.getInitializerData)(adminFactory.interface, args, opts.initializer);
        }
        else {
            mockImplAddress = constants_1.defaultImplAddresses[chainId].contractAddress;
            data = (0, utils_general_1.getInitializerData)(new ethers.Interface(mockArtifact.abi), args, opts.initializer);
        }
