...
exports.checkVerificationStatus = exports.getLibraries = exports.getSolidityStandardJsonInput = exports.getMinimalResolvedFiles = exports.checkContractName = exports.flattenContractFile = exports.inferContractArtifacts = void 0;
...
async function getMinimalResolvedFiles(hre, sourceName) {
    ...
    const sourcePaths = await hre.run(task_names_1.TASK_COMPILE_SOLIDITY_GET_SOURCE_PATHS, { sourcePath: hre.config.paths.sources });
...
function getSolidityStandardJsonInput(hre, resolvedFiles, input) {
    return {
...
async function checkVerificationStatus(args, hre) {
    const isValidVerification = await (0, utils_1.executeVeificationWithRetry)(args.verificationId, hre.network.verifyURL);
    if (isValidVerification?.errorExists()) {
        throw new errors_1.ZkSyncVerifyPluginError(`Backend verification error: ${isValidVerification.getError()}`);
    }
