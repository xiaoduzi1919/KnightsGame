...
async function verify(args, hre, runSuper) {
    ...
    await hre.run(constants_1.TASK_VERIFY_VERIFY, {
...
async function getCompilerVersions(_, hre, runSuper) {
    ...
    const compilerVersions = hre.config.solidity.compilers.map((c) => (0, utils_1.normalizeCompilerVersions)({ compiler: c }, extractedConfigs?.compilers ?? []) ?? c.version);
    if (hre.config.solidity.overrides !== undefined) {
        for (const [file, compiler] of Object.entries(hre.config.solidity.overrides)) {
            compilerVersions.push((0, utils_1.normalizeCompilerVersions)({ compiler, file }, extractedConfigs?.overides ?? new Map()) ??
                compiler.version);
        }
    }
...
async function getConstructorArguments(args, hre, runSuper) {
    ...
    try {
        const constructorArguments = await (0, utils_1.extractModule)(constructorArgsModulePath);
        ...
        if (!Array.isArray(constructorArguments) && !constructorArguments.startsWith('0x')) {
            throw new errors_1.ZkSyncVerifyPluginError((0, constants_1.ENCODED_ARAGUMENTS_NOT_FOUND_ERROR)(constructorArgsModulePath));
        }
        return constructorArguments;
    }
    catch (error) {
        throw new errors_1.ZkSyncVerifyPluginError((0, constants_1.CONSTRUCTOR_MODULE_IMPORTING_ERROR)(error.message), error);
    }
}
...
async function verifyContract({ address, contract: contractFQN, constructorArguments, libraries, noCompile }, hre, runSuper) {
    ...
    const deployedBytecodeHex = await (0, utils_1.retrieveContractBytecode)(address, hre);
    ...
    if (!noCompile) {
        await hre.run(constants_1.TASK_COMPILE, { quiet: true });
    }
    ...
    try {
        await hre.run(constants_1.TASK_CHECK_VERIFICATION_STATUS, { verificationId });
    }
    catch (error) {
        ...
        if (error.message !== constants_1.NO_MATCHING_CONTRACT &&
            constants_1.COMPILATION_ERRORS.filter((compilationError) => compilationError.pattern.test(error.message)).length === 0) {
            throw error;
        }
        console.info(chalk_1.default.red(constants_1.UNSUCCESSFUL_CONTEXT_COMPILATION_MESSAGE));
        ...
        await hre.run(constants_1.TASK_CHECK_VERIFICATION_STATUS, { verificationId: fallbackVerificationId });
    }
...
async function getContractInfo({ contractFQN, deployedBytecode, matchingCompilerVersions, libraries }, hre, runSuper) {
    ...
    if (contractFQN !== undefined) {
        ...
        if (buildInfo === undefined) {
            throw new errors_1.ZkSyncVerifyPluginError((0, constants_1.BUILD_INFO_NOT_FOUND_ERROR)(contractFQN));
        }
