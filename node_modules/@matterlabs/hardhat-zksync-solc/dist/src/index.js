...
(0, config_env_1.extendEnvironment)((hre) => {
    if (hre.network.config.zksync) {
        ...
        hre.config.solidity.compilers.forEach((compiler) => (0, utils_1.updateCompilerConf)({ compiler }, hre.config.zksolc, extractedConfigs?.compilers ?? []));
        for (const [file, compiler] of Object.entries(hre.config.solidity.overrides)) {
            (0, utils_1.updateCompilerConf)({ compiler, file }, hre.config.zksolc, extractedConfigs?.overides ?? new Map());
        }
    }
});
